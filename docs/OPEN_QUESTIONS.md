...
422/422 tests passing. TSC clean. Build clean.
...
19. **A handful of sprites still carry a residual dark halo after the 2026-07-03 de-fringe pass** — `forge.png` (~25% border-ring still dark-translucent even after 5 correction iterations), and to a lesser degree `forge_4x4.png`, `gemcutting_4x4.png`, `hearth_4x4.png` (~5-11%). The unpremultiply-against-black correction helps but doesn't fully resolve these - likely because the affected region is a genuinely wide/soft gradient in the source art (a real design element, e.g. ambient glow or shadow) rather than a thin anti-aliasing fringe, so the correction's conservative floor (to avoid amplifying noise into garish colors) limits how far it can push. Would need either a cleaner regenerated source or manual retouching to fully fix; not resolved here on purpose, to avoid the correction pass damaging legitimate shading elsewhere in those images.

---

## Recently Resolved (changelog)

- **Multi-tile sprite anchor bug** (2026-07-03) — reported as "sprites render out of position, then snap into place as the camera approaches." Root cause: the static grid sets every cell within a structure's footprint (e.g. all 36 cells of the 6×6 Hearth) to the same CellKind - correct for the ASCII renderer, but TilesetRenderer was assuming whatever cell it looked at WAS the sprite's top-left anchor. If the camera scrolled such that an interior cell entered the viewport before the true anchor did, the whole sprite got drawn at the wrong position. Fixed by walking backward via `getCell` (bounded by the sprite's own span) to find the true anchor regardless of which cell triggered the draw - extracted as `findSpriteAnchorOffset`, a pure function with its own test coverage (7 new tests) since TilesetRenderer itself needs a real canvas context to test directly.
- **Floor/torch sprite replacement + widespread dark-halo fix** (2026-07-03) — replaced `rock_floor` and `torch_lit` with new sprites. While auditing why "the torch and many other sprites" looked like they had a black background, found a real systemic issue: partial-alpha edge pixels across most of the tileset retained dark RGB values from black-background blending (classic un-premultiplied-alpha fringing - confirmed my own gemwood/sawmill sprites had introduced a fresh version of this via LANCZOS-resizing a binary alpha mask without premultiplying first). Applied an "unpremultiply against black" correction across all three sprite directories (tileset-assets, skill-icons, tool-icons), iterated until convergence. Most sprites dropped from 20-85% border-halo pixels to under 5%; a few (forge.png in particular) plateaued higher (~7-25%) - stopped rather than risk over-correcting legitimate soft shadow/glow details elsewhere in those images. Flagged below for anyone who wants those specific ones fully clean.
