...
21. **wood_planks cost rebalancing - not started** (2026-07-06) — direct instruction: "This allows us to change building and upgrade costs to wooden planks rather than root wood." Now that the Harvester -> harvest companion -> Sawmill chain exists, wood_planks has real production infrastructure behind it (previously it had zero consumers at all - see the old gap #17, now largely superseded). Only one recipe uses wood_planks so far (the harvest companion's own befriend cost, 10 planks - chosen because it's thematically tidy, not because of any broader plan). The actual ask - auditing existing `wood`-costed recipes across the game and converting some/most of them to `wood_planks` at a sensible ratio - hasn't been touched. Needs a decision on which recipes convert and at what ratio (planks cost more effort per unit than raw wood, so a straight 1:1 swap would be a stealth nerf) before touching this broadly.
