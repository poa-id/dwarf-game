...
19. **Sprite dark-halo/black-background look** — attempted an automated "unpremultiply against black" fix (2026-07-03), reverted the same day per direct feedback ("they look hideous"). The underlying issue (partial-alpha edge pixels retaining dark RGB from black-background blending) is real, but the automated correction wasn't an acceptable fix - visual quality regressed. Back to square one on this one; needs either a different technical approach or per-sprite manual attention, not another blanket automated pass.
...
- **Sprite dark-halo fix, attempted and reverted** (2026-07-03, same day) — see gap #19. The floor/torch sprite replacements from this pass were reverted too, back to the original tileset art.
